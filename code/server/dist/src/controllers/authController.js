...
const login = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        res.status(200).json({ token, moodleAutoLoginUrl, user: { id: user._id, name: user.name, email: user.email, role: user.role } });
        return;
    }
    catch (error) {
        console.error(error);
...
const register = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const newUser = new User_1.User({ name, email, password, role, moodleUserId: userId });
        yield newUser.save();
        // Generate JWT Token
...
//         await user.save();
//         try {
//             const userId = await createMoodleUser(name, email, email.split('@')[0], password);
//             const roleId = roleMapping[role];
//             if (userId) {
...
//             } else {
//                 console.error("Error creating Moodle user:", (error as Error).message);
//             }
//             next(error); 
//         }
//         const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET as string, { expiresIn: "1h" });
